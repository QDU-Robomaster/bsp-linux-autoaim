...
#include "thread.hpp"
#include "xrobot_main_demo.hpp"

void (*demo_log_cb_fun)(bool in_isr, LibXR::Topic, LibXR::RawData &log_data) =
    [](bool, LibXR::Topic tp, LibXR::RawData &log_data)
{
  ...
  if (LibXR::STDIO::write_ && LibXR::STDIO::write_->Writable())
  {
    ...
    if (!f.is_open())
    {
      ...
      oss << std::put_time(&tm, "%Y%m%d_%H%M%S") << "_demo.log";
...
int main(int, char **)
{
  ...
  auto log_cb = LibXR::Topic::Callback::Create(demo_log_cb_fun, log_topic);
