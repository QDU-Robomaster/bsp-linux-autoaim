...
static void XRobotMain(LibXR::HardwareContainer &hw) {
  ...
  static SharedTopic shared_topic_rx(
      ...
      "DevC-USB",
      4096,
      {{"gimbal_gyro", "host"}, {"gimbal_accl", "host"}, {"gimbal_quat", "host"}, {"camera_sync_result", "host"}, {"sentry_ref", "host"}}
  );
  static ArmorDetector<AutoAimRunConfig::HikCameraInfo> armor_detector(
      hw,
      appmgr,
      {2, {0.1, true, 16.0, "AUTO_DETECT", "LATENCY"}, true, "host", "sentry_ref", {false, "armor_detector_preview", 0.5, 1, 1, "window", "0.0.0.0", 8080, "armor_detector", 30.0}, {true, 0.2, 0.9, true}},
      camera_frame_sync
  );
